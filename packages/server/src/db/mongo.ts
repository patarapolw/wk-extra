...
export class DbUser {
  @prop() _id!: string
  @prop() name!: string // Not a strict requirement, but helpful in displaying UX
  @prop() apiKey!: string
  /**
   * @internal
   */
  @prop({ default: () => ({}) }) settings?: Record<string, any>
...
class DbExtra implements IDbExtraExport {
  ...
  @prop({ required: true, validate: (s) => ['jp', 'zh'].includes(s) }) lang!:
    | 'jp'
    | 'zh'
